...
esferes = esferes %>% mutate(risc = ifelse(resultat_tot!='sense risc' | resultat_benestar!='sense risc' | resultat_emocional!='sense risc' | resultat_familiar!='sense risc' | resultat_escolar!='sense risc' | resultat_social!='sense risc' | sol>=3 | social>=3 | no_grup_amics>=2 | casa_bien>=3 | actuar_davant_abus>=3, 'risc', 'no_risc'))
```
...
```{r histogrames_variables, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
...
plot <- ggplot(sum_df, aes(x = variable, y = sum, fill = color_group)) +
...
# Set up the plotting area with two columns
par(mfrow = c(1, 2))
...
ggplot(sum_df, aes(x = variable, y = sum, fill = color_group)) +
  geom_bar(stat = "identity") +
...
* Les següents variables presenten diferències significatives entre sexes segond chi-squared test:
